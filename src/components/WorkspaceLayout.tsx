...
export function WorkspaceLayout() {
  ...
  const quickToolMenuItems = useMemo<QuickToolMenuItem[]>(() => {
    ...
    baseItems.push(
      ...
      },
      {
        ...
        action: () => {
          ...
          window.dispatchEvent(new CustomEvent('foconn:reset-floating-ball-position'));
        },
      },
    );
    ...
    return baseItems;
  }, [isFloatingBallHidden, openLocalTerminalTab, quickToolMenuAnchor, setActiveTab, t]);
  ...
  const shouldIgnoreDashboardContextMenu = (target: EventTarget | null) => {
    ...
    return Boolean(target.closest('button, a, [data-native-contextmenu="true"]'));
  };
    ...
    <div className="flex h-full flex-col bg-[var(--app-bg-base)]">
      <div className="flex h-10 shrink-0 items-center border-b border-[var(--app-border)] bg-[var(--app-bg-container)] px-2">
        <div className="flex min-w-0 flex-1 items-center overflow-x-auto">
          {tabs.map((tab) => {
            ...
            const Icon =
              tab.protocol === 'DASHBOARD' ? LayoutGrid : tab.protocol === 'TERMINAL' ? TerminalSquare : Server;
            const displayTitle =
            ...
            return (
              ...
              >
                <Icon size={14} className={tab.connection ? 'text-[var(--app-primary)]' : 'text-[var(--app-text-muted)]'} />
        ...
        {activeTab.protocol === 'DASHBOARD' ? (
          <div
            ...
            onContextMenu={(event) => {
              ...
              event.preventDefault();
              event.stopPropagation();
              openQuickToolMenu({ x: event.clientX, y: event.clientY }, 'dashboard', 'DASHBOARD');
            }}
          >
            <Home onOpenLocal={openLocalTerminalTab} onOpenProtocolTab={openProtocolTab} />
          </div>
        ) : null}
          ...
          <div className="h-full">
            ...
            >
              <ServerView
                ...
                onTabChange={(tab) => updateServerTabActiveView(activeTab.id, tab)}
              />
            </CrashBoundary>
          </div>
        ) : null}

        {!activeTab.connection && activeTab.protocol !== 'DASHBOARD' && activeTab.protocol !== 'TERMINAL' ? (
          <div className="flex h-full items-center justify-center bg-[var(--app-bg-base)]">
            <div className="rounded-3xl border border-[var(--app-border)] bg-[var(--app-bg-elevated)] px-8 py-10 text-center shadow-[var(--app-shadow)]">
              <div className="text-lg font-semibold text-[var(--app-text-base)]">{t('omnibox.pending_title')}</div>
              <div className="mt-2 text-sm text-[var(--app-text-muted)]">
                {t(`protocols.${activeTab.protocol}`)} · {t('omnibox.pending_desc')}
              </div>
            </div>
          </div>
        ) : null}
      ...
      >
        <OmniboxContent
          ...
          openContext={activeTab.protocol === 'DASHBOARD' ? 'dashboard' : 'workspace'}
        />
      </FoconnPopover>
      ...
      <BookmarkSidebar isOpen={isBookmarkSidebarOpen} onClose={() => setIsBookmarkSidebarOpen(false)} />
      {shouldShowFloatingBall ? (
        <FoconnFloatingBall
          onClick={() => setIsOmniboxOpen(true)}
          onOpenContextMenu={(anchor) =>
            openQuickToolMenu(anchor, 'floating-ball', activeTab.protocol === 'SFTP' ? 'SFTP' : 'SSH')
          }
        />
      ) : null}
      <QuickToolMenu
        isOpen={quickToolMenuAnchor !== null}
        anchor={quickToolMenuAnchor ? { x: quickToolMenuAnchor.x, y: quickToolMenuAnchor.y } : null}
