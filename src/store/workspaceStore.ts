...
function prependHistory(history: HistoryItem[], item: HistoryItem) {
  const next = [item, ...history.filter((entry) => !(entry.protocol === item.protocol && entry.host === item.host && entry.name === item.name))];
...
export const useWorkspaceStore = create<WorkspaceStore>((set) => ({
  ...
  closeTab: (id) =>
    set((state) => {
      ...
      const nextActiveTabId =
        state.activeTabId === id ? nextTabs[nextTabs.length - 1]?.id ?? 'dashboard' : state.activeTabId;
      return {
    ...
    set((state) => {
      ...
      return {
        ...
        history: prependHistory(state.history, {
          ...
          openedAt: Date.now(),
          description: connection.description || `${nextTab.protocol} · ${connection.host}`,
        }),
      };
    }),
  openProtocolTab: (config) =>
      ...
      if (config.protocol === 'SSH' || config.protocol === 'SFTP') {
        ...
        return {
          ...
          history: prependHistory(state.history, {
            ...
            username: config.username,
            openedAt: Date.now(),
            description: config.description || `${config.protocol} · ${config.host ?? ''}`,
          }),
        };
      }
      const id = crypto.randomUUID();
  ...
  loadQuickCommands: async () => {
    try {
      const quickCommands = await invoke<QuickCommandItem[]>('get_quick_commands');
      set({ quickCommands });
    } catch {
      set({ quickCommands: [] });
    }
  ...
  saveBookmark: async (bookmark) => {
    const bookmarkId = await invoke<string>('save_bookmark', { config: bookmark });
    const tree = await invoke<BookmarkGroupWithItems[]>('get_bookmark_tree');
  ...
  deleteBookmark: async (bookmarkId) => {
    ...
    set((state) => ({
      bookmarkTree: tree,
      bookmarkEditor: state.bookmarkEditor?.id === bookmarkId ? null : state.bookmarkEditor,
    }));
  },
  ...
  deleteQuickCommand: async (commandId) => {
    ...
    set((state) => ({
      quickCommands,
      quickCommandEditor: state.quickCommandEditor?.id === commandId ? null : state.quickCommandEditor,
    }));
  },
    ...
    set((state) => {
      ...
      return {
        ...
        history: prependHistory(state.history, {
          ...
          username: bookmark.username,
          openedAt: Date.now(),
          description: bookmark.description || `${bookmark.protocol} · ${bookmark.host}`,
        }),
      };
    }),
}));
